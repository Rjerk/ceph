...
static void dump_inconsistent(const librados::inconsistent_obj_t& inc, Formatter& f)
{
...
extern "C" int _rados_inconsistent_obj_list(rados_ioctx_t io, const char* pg_str, char* buf, size_t size)
{
  tracepoint(librados, rados_inconsistent_obj_list_enter, io, pg_str, size);
  int ret = 0;

  pg_t pgid;
  if (!pgid.parse(pg_str)) {
    return -EINVAL;
  }
  ...
  for (librados::object_id_t start; ; ) {
    ...
    ret = ctx->get_inconsistent_objects(pgid, start, max_item_num,
                    completion->pc, &items, &interval);
